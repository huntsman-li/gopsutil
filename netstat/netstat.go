...
func TCPSocks(accept AcceptFn, path string) ([]SockTabEntry, error) {
	return osTCPSocks(accept, path)
}

// TCP6Socks returns a slice of active TCP IPv4 sockets containing only those
// elements that satisfy the accept function
func TCP6Socks(accept AcceptFn, path string) ([]SockTabEntry, error) {
	return osTCP6Socks(accept, path)
}

// UDPSocks returns a slice of active UDP sockets containing only those
// elements that satisfy the accept function
func UDPSocks(accept AcceptFn, path string) ([]SockTabEntry, error) {
	return osUDPSocks(accept, path)
}

// UDP6Socks returns a slice of active UDP IPv6 sockets containing only those
// elements that satisfy the accept function
func UDP6Socks(accept AcceptFn, path string) ([]SockTabEntry, error) {
	return osUDP6Socks(accept, path)
}
