...
func osTCPSocks(accept AcceptFn, path string) ([]SockTabEntry, error) {
	if path == "" {
		path = pathTCPTab
	}
...
// elements that satisfy the accept function
func osTCP6Socks(accept AcceptFn, path string) ([]SockTabEntry, error) {
	if path == "" {
		path = pathTCPTab
	}
...
// elements that satisfy the accept function
func osUDPSocks(accept AcceptFn, path string) ([]SockTabEntry, error) {
	if path == "" {
		path = pathTCPTab
	}
...
// elements that satisfy the accept function
func osUDP6Socks(accept AcceptFn, path string) ([]SockTabEntry, error) {
	if path == "" {
		path = pathTCPTab
	}
